...
function rcs_manage_enqueue(){
	...
	$adn_priority = 1;
	if(isset($wp_filters['admin_enqueue_scripts'])){
		foreach($wp_filters['admin_enqueue_scripts'] as $key => $val){
	...
	$cln_priority = 1;
	if(isset($wp_filters['wp_enqueue_scripts'])){
		foreach($wp_filters['wp_enqueue_scripts'] as $key => $val){
...
function rcs_admin_styles_scripts(){
	...
	if($post_type == 'rc_slider'){
		...
		//wp_enqueue_script('camera-js', plugins_url('lib/camera/scripts/camera.js', RCS_PLUGIN_MAIN_FILE), array('jquery', 'jquery-effects-core'));
...
function rcs_set_settings(){
	...
	$defaults = array('height' => '50%', 'skin_color' => 'camera_graphite_skin', 'easing' => 'easeInOutExpo', 'fx' => array('random'),
...
function rcs_add_boxes(){
	add_meta_box('slider_options', 'Slider options', 'rcs_build_options_box', 'rc_slider', 'side', 'core');
	add_meta_box('slider_code', 'Slider code', 'rcs_build_code_box', 'rc_slider', 'side', 'core');
	add_meta_box('slider_slides', 'Slides', 'rcs_build_slides_box', 'rc_slider', 'normal', 'high');
	add_meta_box('slider_preview', 'Slider preview', 'rcs_build_preview_box', 'rc_slider', 'normal', 'high');
}
//---------------------------------------------------
/**
...
function rcs_build_preview_box($post){
	if(rcs_count_postmeta($post->ID, '_rcs_slider_slide') > 0){
		...
		$height = $slider_options['height'] + 20;
		switch($slider_options['thumbnail_size']){
			case 'extraLarge':
			$height += 110;
			break;
		...
		echo '<iframe src="' . home_url( '?rcspageid=rcs_slider&id=' . $post->ID ) . '"
				style="border: none; width: 100%; height: 600px; padding: 0; margin: 0;" scrolling="no" ></iframe>';
	}
}
//---------------------------------------------------
/**
...
function rcs_render_preview_slider() {
	...
	<html>
		<head>
			...
			<link rel="stylesheet" type="text/css" href="<?php echo plugins_url('lib/jquery-ui.css', RCS_PLUGIN_MAIN_FILE); ?>" />
			
			<link rel="stylesheet" type="text/css" href="<?php echo plugins_url('lib/camera/css/camera.css', RCS_PLUGIN_MAIN_FILE); ?>" />
			
			<link rel="stylesheet" type="text/css" href="<?php echo plugins_url('css/admin.css', RCS_PLUGIN_MAIN_FILE); ?>" />
			
			<script type="text/javascript" src="<?php echo plugins_url('lib/jquery-1.8.0.min.js', RCS_PLUGIN_MAIN_FILE); ?>"></script>

			<script type="text/javascript" src="<?php echo plugins_url('lib/jquery-ui.js', RCS_PLUGIN_MAIN_FILE); ?>"></script>
			
			<script type="text/javascript" src="<?php echo plugins_url('lib/camera/scripts/jquery.mobile.customized.min.js', RCS_PLUGIN_MAIN_FILE); ?>"></script>
			
			<script type="text/javascript" src="<?php echo plugins_url('lib/camera/scripts/camera.js', RCS_PLUGIN_MAIN_FILE); ?>"></script>
...
function rcs_HideMessageAjaxFunction()
	{  
		add_option( 'rcs_upgrade_message','yes');
	}  



function rcs_after_plugin_row($plugin_file, $plugin_data, $status) {
	        
			if(get_option('rcs_upgrade_message') !='yes')
			{
				...
				echo 'You are running GIGA LITE. To get more features, you can <a href="http://www.wp-buy.com/product/giga-slider-pro" target="_blank"><strong>upgrade now</strong></a>  ';
			    ...
			    jQuery(document).ready(function() {
					...
					jQuery("#HideMe").click(function(){ 
					  jQuery.ajax({  
							...
							data: {  
								action: 'rcs_HideMessageAjaxFunction'
							},  
